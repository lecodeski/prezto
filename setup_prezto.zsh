...
"${ZDOTDIR:-$HOME}"/.zprezto/setup_links_runcoms.zsh || { echo "ERROR: linking runcoms failed, aborting" >&2; exit 1; }
...
if sudo -v; then
  grep -qxF "$zpath" /etc/shells || echo "\n$zpath" | sudo tee -a /etc/shells >/dev/null
  me=$(id -un)
  sudo chsh -s "$zpath" "$me" || chsh -s "$zpath" || sudo usermod -s "$zpath" "$me" || echo "WARN: could not set login shell to $zpath" >&2
  ...
  if [[ $OSTYPE != darwin* ]]; then
    ...
    if [[ -d $sshd_config_d ]]; then
      ...
      sudo sshd -T | grep -qi terminal_intellij || echo "WARN: AcceptEnv not effective, check sshd running and Include in sshd_config" >&2
    else
