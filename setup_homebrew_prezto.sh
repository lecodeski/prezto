...
[[ $OSTYPE == darwin* && $(sysctl -n sysctl.proc_translated 2>/dev/null) == 1 ]] &&
  { echo "ERROR: Rosetta shell, run from a native arm64 terminal" >&2; exit 1; }

script=$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh) && [[ $script ]] &&
  /bin/bash -c "$script" ||
...
[[ -x $HOMEBREW_PREFIX/bin/brew ]] ||
  { echo "ERROR: no brew at $HOMEBREW_PREFIX, install failed?" >&2; exit 1; }

eval "$("$HOMEBREW_PREFIX"/bin/brew shellenv)"

brew bundle --file="${ZDOTDIR:-$HOME}"/.zprezto/setup_Brewfile

[[ -x $HOMEBREW_PREFIX/bin/zsh ]] ||
  { echo "ERROR: no zsh at $HOMEBREW_PREFIX/bin/zsh, aborting" >&2; exit 1; }
